...
export const Route = createFileRoute("/_authenticated/admin")({
  head: () => ({
    meta: [
      { title: "Bookings — Danny Graphics Admin" },
...
function AdminPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await callEnsureAdmin();
      if (!res.isAdmin) {
        setAuthorized(false);
        return;
      }
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-5">
        <div className="glass max-w-md rounded-3xl p-8 text-center shadow-elegant">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            This account doesn't have admin access to the bookings dashboard.
          </p>
          <button
            onClick={signOut}
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          <div>
            <h1 className="font-display text-lg font-bold">Bookings</h1>
            <p className="text-xs text-muted-foreground">Danny Graphics admin</p>
          </div>
